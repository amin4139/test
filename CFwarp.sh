...
ud4='sed -i "4 s/^/PostUp = ip -4 rule add from '$(ip route get 162.159.193.1 | grep -oP 'src \K\S+')' lookup main\n/;5 s/^/PostDown = ip -4 rule delete from '$(ip route get 162.159.193.1 | grep -oP 'src \K\S+')' lookup main\n/" /etc/wireguard/wgcf.conf'
ud6='sed -i "6 s/^/PostUp = ip -6 rule add from '$(ip route get 2606:4700:d0::a29f:c001 | grep -oP 'src \K\S+')' lookup main\n/;7 s/^/PostDown = ip -6 rule delete from '$(ip route get 2606:4700:d0::a29f:c001 | grep -oP 'src \K\S+')' lookup main\n/" /etc/wireguard/wgcf.conf'
ud4ud6='sed -i "4 s/^/PostUp = ip -4 rule add from '$(ip route get 162.159.193.1 | grep -oP 'src \K\S+')' lookup main\n/;5 s/^/PostDown = ip -4 rule delete from '$(ip route get 162.159.193.1 | grep -oP 'src \K\S+')' lookup main\n/;6 s/^/PostUp = ip -6 rule add from '$(ip route get 2606:4700:d0::a29f:c001 | grep -oP 'src \K\S+')' lookup main\n/;7 s/^/PostDown = ip -6 rule delete from '$(ip route get 2606:4700:d0::a29f:c001 | grep -oP 'src \K\S+')' lookup main\n/" /etc/wireguard/wgcf.conf'
